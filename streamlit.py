...
def get_date_filters(df, view_type="Monthly"):
    if view_type == "Monthly":
        ...
        months_in_year_list = list(months_in_year)
        default_month_index = months_in_year_list.index(latest_month_name) if latest_month_name in months_in_year_list else 0
        selected_month_name = st.sidebar.selectbox("Select Month:", months_in_year_list, index=default_month_index)
...
def show_upload_page():
    ...
    st.session_state['salary'] = st.number_input("Monthly Salary ($):", min_value=0.0, value=5000.0, step=100.0)

def show_manage_page():
    st.markdown("<div class='main-header'><h1>SpendWise</h1></div>", unsafe_allow_html=True)
    if not st.session_state.get('df_spent') is not None:
        df_spent = st.session_state['df_spent']
    else:
        st.warning("Upload your bank statement first.")
        return
...
def show_analyze_page():
    st.markdown("<div class='main-header'><h1>SpendWise</h1></div>", unsafe_allow_html=True)
    if not st.session_state.get('df_spent') is None:
        df_spent = st.session_state['df_spent']
    else:
        st.warning("Upload your bank statement first.")
        return
    ...
    if not df_filtered.empty:
        ...
        with col2:
            ...
            if view_type == "Monthly":
                ...
                fig_line = px.line(weekly_data, x='Week', y='Amount', markers=True, color_discrete_sequence=['#5cb85c'])
            else:
                ...
                fig_line = px.line(monthly_data, x='MonthName', y='Amount', markers=True, color_discrete_sequence=['#5cb85c'])
...
def show_predict_page():
    ...
    future_months = np.array(range(X[-1][0]+1, X[-1][0]+13)).reshape(-1,1)
    future_preds = model.predict(future_months)

    months_labels = pd.date_range(df_sorted['Date'].max() + pd.offsets.MonthBegin(1), periods=12, freq='MS').strftime('%b %Y')
    forecast_df = pd.DataFrame({'Month': months_labels, 'Predicted Spending': future_preds})
    forecast_df['Predicted Savings'] = salary - forecast_df['Predicted Spending']

    st.bar_chart(forecast_df.set_index('Month')[['Predicted Spending','Predicted Savings']])
...
def main():
    ...
    if page == "Upload": show_upload_page()
    elif page == "Manage": show_manage_page()
    elif page == "Analyze": show_analyze_page()
    elif page == "Predict": show_predict_page()
